...
function Icon({ glyph, focused }: { glyph: string; focused: boolean }) {
  return (
    <View style={[s.icon, focused && s.iconOn]}>
      <Text style={[s.glyph, focused && s.glyphOn]}>{glyph}</Text>
    </View>
  );
}
  ...
  return (
    <Tabs
      screenOptions={{
        ...
        tabBarLabelStyle: { fontSize: 10.5, fontWeight: '800', paddingBottom: 5 },
        tabBarStyle: {
          height: 82,
          paddingTop: 7,
    ...
    >
      ...
      <Tabs.Screen name="profile" options={{ title: 'Mon espace', tabBarIcon: ({ focused }) => <Icon glyph="●" focused={focused} /> }} />
...
const s = StyleSheet.create({
  ...
  iconOn: { backgroundColor: colors.lilac },
  glyph: { fontSize: 18, color: colors.muted, fontWeight: '900' },
  glyphOn: { color: colors.purpleDark },
});
